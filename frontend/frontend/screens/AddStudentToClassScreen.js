...
import { colors, fontFamilies, radii, shadow, spacing } from '../constants/theme.js';
...
// Screen
// ---------------------------------------------------------------------------
export default function AddStudentToClass({ route, navigation }) {
  ...
  const addStudent = useCallback(async (student) => {
    ...
    try {
      await api.post(`/add_student/${course.id}/`, { student_id: student.id });
      setAddedIds(prev => new Set(prev).add(student.id));
    } catch (err) {
      console.error(err);
      Alert.alert('Something went wrong', 'Could not add that student. Please try again.');
    } finally {
      setAddingId(null);
    }
  }, [course.id]);
