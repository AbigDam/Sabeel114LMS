...
// -----------------------------------------------------------------------------
// Browse all users, grouped into three sections: Teachers / Parents / Students.
// Tapping a card opens that user's detail page:
//   navigation.navigate('UserDetail', { type: 'teacher' | 'parent' | 'student', id })
// The trash icon on each card deletes the user via DELETE /delete_user/<id>/.
// -----------------------------------------------------------------------------
...
function UserCard({ user, onPress, onDelete }) {
  const detail = roleDetail(user);
  const isStudent = user.role === ROLE_STUDENT;
  const genderLabel = isStudent ? genderLabelOf(user) : null;

  return (
    <View style={styles.card}>
      <Pressable onPress={onPress} accessibilityRole="button" accessibilityLabel={`Open ${fullNameOf(user)}`}>
      ...
      <Pressable
        ...
        accessibilityLabel={`Delete ${fullNameOf(user)}`}
      >
...
function UserGroupSection({ title, icon, role, users, loading, onUserPress, onUserDelete }) {
...
export default function UserListScreen({ navigation }) {
  ...
  // Refresh whenever the screen regains focus (e.g. after editing/removing a
  // user from their detail page).
  useFocusEffect(
    useCallback(() => {
      loadUsers();
    }, [loadUsers]),
  ...
  function handleUserPress(user) {
    const group = GROUPS.find((g) => g.role === user.role);
    if (!group) return;
    navigation.navigate('UserDetail', { type: group.key, id: user.id });
  }
  ...
  async function handleUserDelete(user) {
    ...
    try {
      await apiCall('delete', `delete_user/${user.id}/`);
    } catch (err) {
      console.error(err);
...
// -----------------------------------------------------------------------------
// GET /current_user_details/ -> list of all teachers, parents, and students
//   [{ id, username, first_name, last_name, email, role, gender,
//      classes_teacher, children, classes_student }]
